...
/**
 * IEC-style AND gate implementation.
 */
public class AndGate extends GateComponent {

  /**
   * Creates an AND gate instance.
   *
   * @param x initial x position
   * @param y initial y position
   * @param color primary stroke color
   */
  public AndGate(double x, double y, Color color) {
    super(x, y, color);
  }
  ...
  public void render() {
    ...
    Text andSymbol = new Text(15, 40, "&");
    andSymbol.setFill(color);
    andSymbol.setFont(Font.font("Arial", FontWeight.BOLD, 25));

    if (isInverting()) {
      Circle dot = new Circle(55, PIN_SPACING, 7);
      dot.setFill(color);
      componentModel.getChildren().addAll(iecGateBody, andSymbol, dot);
    } else {
      componentModel.getChildren().addAll(iecGateBody, andSymbol);
    }
